...
// Backend response types (actual format from backend)
...
class ApiClient {
  ...
  async login(email: string, password: string): Promise<ApiResponse> {
    const response = await this.post<BackendAuthResponse>('/api/auth/login', { email, password });

    // Backend returns {message, token, user} format, convert to our ApiResponse format
    if (response.token) {
  ...
  async register(userData: {
    ...
    const response = await this.post<BackendAuthResponse>('/api/auth/register', userData);
  ...
  async getCurrentUser(): Promise<ApiResponse> {
    // Backend doesn't have a /me endpoint, so we'll just check if we have a valid token
    if (!this.token) {
      throw new Error('No authentication token. Please login first.');
    }
    
    // For now, return success with token validation only
    // In a real app, you'd call an actual /me endpoint
    return {
      success: true,
